...
module Lulz
	class TwoTermGoogleAgent < Agent
		PAGES=2
      #also_accept_on :homepage
      default_process :google
      searcher
      set_description "search google for objects with second term i.e. dirtyfilthy lulz"
		one_at_a_time      
      def self.accepts?(pred)
         subject=pred.subject
	 object=pred.object
	 return false unless pred.name==:second_google_term
	 return false if pred.object.is_a?(Country) or pred.object.is_a?(Locality) or object.to_s.length==2
